...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  // const res = Math.abs(Math.atan2(y2, x2)) - Math.abs(Math.atan2(y1, x1));
  // if (res === 0) {
  //   return 3.141592653589793;
  // }
  // return res * -1;
  const middle = x1 * x2 + y1 * y2;
  const res1 = Math.sqrt(x1 * x1 + y1 * y1);
  const res2 = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(middle / (res1 * res2));
}
...
function getSumOfDigits(num) {
  const numMass = num.toString().split('');
  let res = 0;
  for (let i = 0; i < numMass.length; i += 1) {
    res += +numMass[i];
  }
  return res;
}
...
function isPowerOfTwo(num) {
  const mass = [1, 2];
  for (let i = 2; mass[i - 1] <= num; i += 1) {
    mass[i] = mass[i - 1] * 2;
  }
  if (mass[mass.length - 2] === num) {
    return true;
  }
  return false;
}
...
function isNumber(number) {
  ...
  return Number.isFinite(number) && number === parseInt(number, 10);
}
